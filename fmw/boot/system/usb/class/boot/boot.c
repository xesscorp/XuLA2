...
{
	big_counter = *(word*)(&dataPacket.ADR);
    for (counter = 0; counter < dataPacket.len; counter++,big_counter++)
    {
//
// This was added so that the PICDEM FS USB Tool would not complain when
// that flash erasure failed when using an 18F4455 with only 24K prgram memory.
//
